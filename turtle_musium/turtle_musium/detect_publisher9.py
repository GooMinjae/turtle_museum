...
class YoloPerson(Node):
    def __init__(self):
        ...
        # ---- YOLO (모델 단일 로드) ----
        self.model = YOLO('/home/rokey/turtlebot4_ws/src/turtle_musium/resource/all_detect.pt')
        self.conf = 0.7
    ...
    def callback_giftshop(self, msg: Bool):
        ...
        if not self.gift_client.wait_for_service(timeout_sec=0.0):
    ...
    def _on_gift_response(self, future):
        try:
            ...
            self._gift_inflight = False
            self.get_logger().error(f"서비스 응답 에러: {e}")
            return
        ...
        raw = result.message.strip()  # 예: "0110"
        # 길이 보정
        if len(raw) < len(self.gift_name):
    ...
    def cb_depth(self, msg: Image):
        ...
        with self.lock:
            # RGB가 이미 있으면, 그 크기에 맞춰 Depth를 리사이즈
            if self.last_rgb is not None:
                H_rgb, W_rgb = self.last_rgb.shape[:2]
                # (필요시: FOV 보정이 필요하면 여기서 crop 후 resize 로직 추가)
...
#         # === Load YOLO model ===
#         self.model = YOLO(
#             "/home/rokey/turtlebot4_ws/src/turtle_musium/resource/all_detect.pt"
#         )
#         self.model.to('cuda')
#         self.get_logger().info("YOLOv8 model loaded.")
...
#                 # 이미 저장된 동일 라벨보다 conf 높으면 교체
#                 if label not in best_boxes_by_label or conf > best_boxes_by_label[label].conf[0]:
#                     best_boxes_by_label[label] = det

#             # 
#             for label, det in best_boxes_by_label.items():
#                 conf = float(det.conf[0])
#                 x1, y1, x2, y2 = map(int, det.xyxy[0].tolist())
