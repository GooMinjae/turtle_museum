...
class YoloPersonNavGoal(Node):
    def __init__(self):
        ...
        # Load YOLOv8 model
        self.model = YOLO("/home/rokey/train_ws/src/yolov8n.pt")
...
# this code is a ROS 2 node that uses YOLOv8 to detect people in RGB images and navigate to their positions using depth information.
# It subscribes to RGB and depth images, processes them to find people, calculates their 3D positions, and sends navigation goals to the detected person's location in the map frame.
# The node also includes a GUI for displaying the detection results and allows manual reset of goal updates.
## The node uses threading to handle image display and processing concurrently, and it integrates with the Nav2